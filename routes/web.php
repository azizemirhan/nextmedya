...
Route::prefix('api')->group(function () {
    Route::post('/validate-coupon', [\App\Http\Controllers\Frontend\CouponController::class, 'validateCoupon'])
        ->middleware('throttle:30,1')
        ->name('api.coupon.validate');
    
    Route::get('/coupon-stats', [\App\Http\Controllers\Frontend\CouponController::class, 'getCouponStats'])
        ->middleware('throttle:60,1')
        ->name('api.coupon.stats');
});
...
// Ana route (throttle ile)
Route::post('/contact', [\App\Http\Controllers\Frontend\ContactController::class, '__invoke'])
    ->middleware('throttle:10,1')
    ->name('frontend.contact.submit');

// Türkçe route alias
Route::post('/iletisim', [\App\Http\Controllers\Frontend\ContactController::class, '__invoke'])
    ->middleware('throttle:10,1')
    ->name('contact.submit');

// Admin panelden gelen varsayılan URL
Route::post('/submit-contact-form', [\App\Http\Controllers\Frontend\ContactController::class, '__invoke'])
    ->middleware('throttle:10,1')
    ->name('contact.form.submit');


// Hizmet Talebi Formu (Bilgi Al)
Route::get('/bilgi-al', [\App\Http\Controllers\Frontend\ServiceRequestController::class, 'show'])
    ->name('service-request.show');

Route::post('/bilgi-al', [\App\Http\Controllers\Frontend\ServiceRequestController::class, 'store'])
    ->middleware('throttle:5,1')
    ->name('service-request.submit');

// Toplantı Planlama Formu
Route::get('/toplanti-planla', [\App\Http\Controllers\Frontend\MeetingRequestController::class, 'show'])
    ->name('meeting-request.show');

Route::post('/toplanti-planla', [\App\Http\Controllers\Frontend\MeetingRequestController::class, 'store'])
    ->middleware('throttle:5,1')
    ->name('meeting-request.submit');
