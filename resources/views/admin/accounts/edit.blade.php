    ...
    <div class="middle-content container-xxl p-0">
                    ...
                    <div class="d-flex breadcrumb-content">
                        <div class="page-header">
                            <div class="page-title">
                                <h5 class="mb-0">Şirketi Düzenle</h5>
                            </div>
                        ...
                        <li class="nav-item more-dropdown">
                            <div class="dropdown custom-dropdown-icon">
                                ...
                                <div class="dropdown-menu dropdown-menu-right">
                                    <a class="dropdown-item" href="{{ route('admin.accounts.preview', $account) }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none"
                                            stroke="currentColor" stroke-width="2" stroke-linecap="round"
        ...
        <form action="{{ route('admin.accounts.update', $account) }}" method="POST" enctype="multipart/form-data">
            ...
            @php
                ...
                $emailsCount = max(count($emails), 1);
                $phonesCount = max(count($phones), 1);
            @endphp
                    ...
                    <div class="user-profile">
                        <div class="widget-content widget-content-area">
                            ...
                            <div class="text-center user-info">
                                <img id="logo-preview" src="{{ $account->logo_path ? asset($account->logo_path) : '#' }}"
                                    alt="logo"
                                    style="{{ $account->logo_path ? '' : 'display:none;' }} max-width:160px; border-radius:8px;">
                                ...
                                <div class="">
                                    <ul class="contacts-block list-unstyled">
                                        <li class="contacts-block__item">
                                            <label class="form-label mb-1">Logo Yükle (Değiştir)</label>
                                        ...
                                        <li class="list-inline-item mb-0">
                                            <select name="status" class="form-select">
                                                @foreach (['active' => 'Aktif', 'inactive' => 'Pasif', 'prospect' => 'Aday', 'churned' => 'Kaybedildi'] as $k => $v)
                                                    <option value="{{ $k }}" @selected(old('status', $account->status) === $k)>
                                                        {{ $v }}</option>
                                                @endforeach
                                            </select>
                                        </li>
                                        <li class="list-inline-item mb-0">
                                            <select name="lifecycle_stage" class="form-select">
                                                @foreach (['lead' => 'Lead', 'customer' => 'Müşteri', 'partner' => 'Partner', 'vendor' => 'Tedarikçi', 'other' => 'Diğer'] as $k => $v)
                                                    <option value="{{ $k }}" @selected(old('lifecycle_stage', $account->lifecycle_stage) === $k)>
                                                        {{ $v }}</option>
                                                @endforeach
                                            </select>
                                        </li>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="registration_no" class="form-control"
                                    value="{{ old('registration_no', $account->registration_no) }}">
                            </div>
                            <div class="col-md-6 mb-3">
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <input type="text" name="source" class="form-control"
                                    placeholder="webform / referans / ads ..."
                                    value="{{ old('source', $account->source) }}">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="owner_id" class="form-select">
                                    ...
                                    @foreach ($owners as $owner)
                                        <option value="{{ $owner->id }}" @selected(old('owner_id', $account->owner_id) == $owner->id)>
                                            {{ $owner->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" name="score" class="form-control" min="0" max="100"
                                    value="{{ old('score', $account->score) }}">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Son Temas</label>
                                <input type="datetime-local" name="last_contacted_at" class="form-control"
                                    value="{{ old('last_contacted_at', optional($account->last_contacted_at)->format('Y-m-d\TH:i')) }}">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Sonraki Aktivite</label>
                                <input type="datetime-local" name="next_activity_at" class="form-control"
                                    value="{{ old('next_activity_at', optional($account->next_activity_at)->format('Y-m-d\TH:i')) }}">
                            </div>
                        </div>
                    </div>

                    <div class="d-flex gap-2">
                        <a href="{{ route('admin.accounts.index', $account) }}"
        ...
        (function() {
            ...
            function formDataToJson(form) {
                ...
                fd.forEach((value, key) => {
                    ...
                    // emails[0][value] gibi nested key'leri nesneye dönüştür
