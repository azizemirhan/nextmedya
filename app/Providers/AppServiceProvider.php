        ...
        try {
            ...
            View::composer('*', function ($view) {
                ...
                if (Auth::check()) {
                    $user = Auth::user();
                    $userData = [
                        'em' => gtm_hash($user->email),      // Email (hashed)
                        'ph' => gtm_hash($user->phone),      // Telefon (hashed) - veritabanınızda phone sütunu varsa
                        'fn' => gtm_hash($user->name),       // İsim (hashed)
                        'db' => gtm_hash($user->birth_date), // Doğum tarihi (varsa)
                        'ct' => gtm_hash($user->city),       // Şehir (varsa)
                        'external_id' => $user->id,          // CRM ID'si
                    ];
                }
        
                $view->with('gtmUser', $userData);
            });
