...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('posts', function (Blueprint $table) {
            ...
            $table->string('title'); // Yazının ana başlığı (H1 etiketi için)
            $table->string('slug')->unique(); // SEO dostu URL
            $table->longText('content'); // Yazının ana içeriği
            $table->text('excerpt')->nullable(); // Yazı özeti (Meta description için fallback)
            $table->string('featured_image')->nullable(); // Öne çıkan görselin dosya yolu
            $table->string('featured_image_alt_text')->nullable(); // Görsel için Alt metni (SEO için çok önemli)
            ...
            // --- SEO Alanları ---
            $table->string('seo_title')->nullable(); // Google'da görünecek özel başlık (<title> etiketi için)
            $table->text('meta_description')->nullable(); // Google'da görünecek özel açıklama
            $table->text('keywords')->nullable(); // Anahtar kelimeler (virgülle ayrılmış)
